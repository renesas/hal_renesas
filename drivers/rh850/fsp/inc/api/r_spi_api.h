...
typedef enum e_spi_event
{
    ...
    SPI_EVENT_DATA_CONSISTENCY,        ///< Data consistency error
    SPI_EVENT_RECEIVE_COMPLETE,        ///< Receive complete
    SPI_EVENT_FIFO_TIMEOUT,            ///< FIFO timeout error
    SPI_EVENT_FIFO_OVERFLOW,           ///< FIFO overflow error
    SPI_EVENT_JOB_ABORT,               ///< Job mode abort
    SPI_EVENT_CRC_ERROR                ///< Crc error
} spi_event_t;
...
typedef struct st_spi_api
{
    ...
    /**
     * Select chip select for a specific SPI device.
     *
     * @param[in]  p_ctrl       Pointer to the control block for the channel.
     * @param[in]  chip_select  Chip select line to enable.
     */
    fsp_err_t (* chipSelect)(spi_ctrl_t * const p_ctrl, uint32_t chip_select);
