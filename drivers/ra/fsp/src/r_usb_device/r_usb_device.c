 ...
 */
fsp_err_t R_USBD_EdptOpen (usbd_ctrl_t * const p_api_ctrl, usbd_desc_endpoint_t const * ep_desc)
{
    ...
    const uint8_t  xfer    = ep_desc->Attributes.xfer;
    ...
    R_USB_HS0->PIPEMAXP = get_edpt_packet_size(ep_desc);
...
static inline uint16_t get_edpt_packet_size (usbd_desc_endpoint_t const * desc_ep)
{
    return (uint16_t) desc_ep->wMaxPacketSize & R_USB_HS0_PIPEMAXP_MXPS_Msk;
}
...
static inline void control_pipe_setup (usbd_instance_ctrl_t * const p_ctrl)
{
    ...
    R_USB_HS0->INTSTS0        = 0;
    R_USB_HS0->INTENB0        = R_USB_HS0_INTSTS0_VBINT_Msk | R_USB_HS0_INTSTS0_BRDY_Msk | R_USB_HS0_INTSTS0_BEMP_Msk |
                                R_USB_HS0_INTSTS0_DVST_Msk | R_USB_HS0_INTSTS0_CTRT_Msk |
