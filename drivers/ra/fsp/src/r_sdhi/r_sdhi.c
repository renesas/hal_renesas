...
#define SDHI_PRV_ACCESS_ERROR_MASK                                             \
  (SDHI_PRV_CARD_CMD_ERR | SDHI_PRV_CARD_CRC_ERR | SDHI_PRV_CARD_END_ERR |     \
   SDHI_PRV_CARD_DTO | SDHI_PRV_CARD_ILW | SDHI_PRV_CARD_ILR |                 \
   SDHI_PRV_CARD_RSPT | SDHI_PRV_CARD_ILA_ERR)

/* The clock register can be accessed 8 SD clock cycles after the last command
 * completes. */
 ...
 * Do not set BREM or BWEM when using DMA/DTC. This driver always uses DMA or
 * DTC. */
#define SDHI_PRV_SDHI_INFO2_MASK_CMD_SEND (0x00007C80U)

/* The relationship of the SD Clock Control Register SD_CLK_CTRL CLKSEL to the
 * division of the source PCLK b7            b0 1 1 1 1 1 1 1 1: PCLK 0 0 0 0 0
 * 0 0 0: PCLK/2 0 0 0 0 0 0 0 1: PCLK/4 0 0 0 0 0 0 1 0: PCLK/8 0 0 0 0 0 1 0
 * 0: PCLK/16 0 0 0 0 1 0 0 0: PCLK/32 0 0 0 1 0 0 0 0: PCLK/64 0 0 1 0 0 0 0 0:
 * PCLK/128 0 1 0 0 0 0 0 0: PCLK/256 1 0 0 0 0 0 0 0: PCLK/512. Other settings
 * are prohibited.
 */
#define SDHI_PRV_MAX_CLOCK_DIVISION_SHIFT                                      \
  (9U) /* 512 (2^9) is max clock division supported */
...
/* Delay up to 10 ms before timing out waiting for response or response timeout
 * flag. */
#define SDHI_PRV_RESPONSE_TIMEOUT_US (10000U)

/* Delay up to 5 seconds before timing out waiting for busy after updating bus
 * width or high speed status for eMMC. */
...
/* Delay up to 100 ms before timing out waiting for access end flag after
 * receiving data during initialization. */
...
#define SDHI_PRV_MIN_CYCLES_PER_COMMAND_RESPONSE                               \
  ((SDHI_PRV_BITS_PER_COMMAD + SDHI_PRV_BITS_PER_RESPONSE) +                   \
...
typedef void(BSP_CMSE_NONSECURE_CALL *sdhi_prv_ns_callback)(
    sdmmc_callback_args_t *p_args);
#elif defined(__GNUC__)
typedef BSP_CMSE_NONSECURE_CALL void (*volatile sdhi_prv_ns_callback)(
    sdmmc_callback_args_t *p_args);
#endif
...
static fsp_err_t r_sdhi_open_param_check(sdhi_instance_ctrl_t *p_ctrl,
...
static fsp_err_t r_sdhi_csd_extended_get(sdhi_instance_ctrl_t *const p_ctrl,
                                         uint32_t rca, uint8_t *p_device_type);
...
static fsp_err_t r_sdhi_cmd52(sdhi_instance_ctrl_t *const p_ctrl,
                              uint8_t *const p_data, uint32_t const function,
...
static fsp_err_t r_sdhi_bus_width_set(sdhi_instance_ctrl_t *const p_ctrl,
                                      uint32_t rca);
...
static void r_sdhi_access_irq_process(sdhi_instance_ctrl_t *p_ctrl,
                                      sdmmc_callback_args_t *p_args);

static void r_sdhi_command_send_no_wait(sdhi_instance_ctrl_t *p_ctrl,
                                        uint32_t command, uint32_t argument);

static fsp_err_t r_sdhi_command_send(sdhi_instance_ctrl_t *p_ctrl,
                                     uint32_t command, uint32_t argument);

static fsp_err_t r_sdhi_max_clock_rate_set(sdhi_instance_ctrl_t *p_ctrl,
                                           uint32_t max_rate);

static fsp_err_t r_sdhi_wait_for_event(sdhi_instance_ctrl_t *const p_ctrl,
                                       uint32_t bit, uint32_t timeout);

static fsp_err_t r_sdhi_rca_get(sdhi_instance_ctrl_t *const p_ctrl,
                                uint32_t *p_rca);
...
static void r_sdhi_read_write_common(sdhi_instance_ctrl_t *const p_ctrl,
                                     ...
                                     uint32_t sector_size, uint32_t command,
                                     uint32_t argument);

static fsp_err_t r_sdhi_transfer_read(sdhi_instance_ctrl_t *const p_ctrl,
                                      uint32_t block_count, uint32_t bytes,
                                      void *p_data);
...
static void r_sdhi_call_callback(sdhi_instance_ctrl_t *p_ctrl,
                                 sdmmc_callback_args_t *p_args);
...
fsp_err_t R_SDHI_Open(sdmmc_ctrl_t *const p_api_ctrl,
  ...
  /* Open the transfer driver. Clear the transfer length in case the
   * transfer_info_t structure is reused and the length was copied to the upper
   * 8 bits for block mode. Configurations are updated before it is used. */
  p_cfg->p_lower_lvl_transfer->p_cfg->p_info->transfer_settings_word = 0U;
  err = p_cfg->p_lower_lvl_transfer->p_api->open(
      p_cfg->p_lower_lvl_transfer->p_ctrl, p_cfg->p_lower_lvl_transfer->p_cfg);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
fsp_err_t R_SDHI_Read(sdmmc_ctrl_t *const p_api_ctrl, uint8_t *const p_dest,
  ...
  err = r_sdhi_transfer_read(p_ctrl, sector_count, p_ctrl->p_cfg->block_size,
                             p_dest);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
  ...
  r_sdhi_read_write_common(p_ctrl, sector_count, p_ctrl->p_cfg->block_size,
                           command, argument);
...
fsp_err_t R_SDHI_Write(sdmmc_ctrl_t *const p_api_ctrl,
  ...
  FSP_ERROR_RETURN(!p_ctrl->device.write_protected,
                   FSP_ERR_CARD_WRITE_PROTECTED);

  /* Configure the transfer interface for writing. */
  err = r_sdhi_transfer_write(p_ctrl, sector_count, p_ctrl->p_cfg->block_size,
                              p_source);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       *                                           ongoing.
                                                                                                                       **********************************************************************************************************************/
fsp_err_t R_SDHI_ReadIo(sdmmc_ctrl_t *const p_api_ctrl, uint8_t *const p_data,
  ...
  *p_data = 0U;
  err = r_sdhi_cmd52(p_ctrl, p_data, function, address,
                     SDMMC_IO_WRITE_MODE_NO_READ, SDHI_PRV_SDIO_CMD52_READ);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       *                                           ongoing.
                                                                                                                       **********************************************************************************************************************/
fsp_err_t R_SDHI_WriteIo(sdmmc_ctrl_t *const p_api_ctrl, uint8_t *const p_data,
                         uint32_t const function, uint32_t const address,
                         sdmmc_io_write_mode_t const read_after_write) {
#if SDHI_CFG_SDIO_SUPPORT_ENABLE
  sdhi_instance_ctrl_t *p_ctrl = (sdhi_instance_ctrl_t *)p_api_ctrl;
  ...
  err = r_sdhi_cmd52(p_ctrl, p_data, function, address, read_after_write,
...
fsp_err_t R_SDHI_ReadIoExt(sdmmc_ctrl_t *const p_api_ctrl,
                           uint8_t *const p_dest, uint32_t const function,
                           uint32_t const address, uint32_t *const count,
  ...
  argument.cmd_53_arg.function_number =
      (function & SDHI_PRV_SDIO_CMD52_CMD53_FUNCTION_MASK);
  argument.cmd_53_arg.block_mode = transfer_mode;
  argument.cmd_53_arg.op_code = address_mode;
  argument.cmd_53_arg.register_address =
      (address & SDHI_PRV_SDIO_CMD52_CMD53_ADDRESS_MASK);
  argument.cmd_53_arg.rw_flag = 0U;
  r_sdhi_read_write_common(p_ctrl, block_count, byte_count, command,
                           argument.arg);
...
fsp_err_t R_SDHI_Erase(sdmmc_ctrl_t *const p_api_ctrl,
  ...
  if (true == p_ctrl->sector_addressing) {
    ...
    end_address =
        ((start_sector + sector_count) * p_ctrl->p_cfg->block_size) - 1U;
  }
...
fsp_err_t R_SDHI_CallbackSet(sdmmc_ctrl_t *const p_api_ctrl,
  ...
  sdmmc_callback_args_t *const p_callback_memory_checked =
...
fsp_err_t R_SDHI_Close(sdmmc_ctrl_t *const p_api_ctrl) {
  ...
  p_ctrl->p_cfg->p_lower_lvl_transfer->p_api->close(
      p_ctrl->p_cfg->p_lower_lvl_transfer->p_ctrl);

  /* Do not set the module stop bit since the CMD0 may not be complete yet. Do
   * not wait for CMD0 to complete because the card could be unplugged and
   * waiting for the response timeout in this function is not desireable. */
...
static fsp_err_t r_sdhi_open_param_check(sdhi_instance_ctrl_t *p_ctrl,
  ...
  FSP_ERROR_RETURN(
      0U != ((1U << p_cfg->channel) & BSP_FEATURE_SDHI_VALID_CHANNEL_MASK),
  ...
  /* Check block size, 512 bytes is the maximum block size the peripheral
   * supports */
  FSP_ASSERT(0U != p_cfg->block_size);
  FSP_ASSERT(p_cfg->block_size <= SDHI_MAX_BLOCK_SIZE);
#else

  /* SD and eMMC cards only support block size of 512 bytes on the SDHI
   * hardware. */
  /* This can't be checked until we know it's not an SDIO card if SDIO is
   * enabled. */
...
static fsp_err_t r_sdhi_erase_error_check(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  /* Check for valid sector count.  Must be a non-zero multiple of
   * erase_sector_count. */
...
static fsp_err_t r_sdhi_common_error_check(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  /* To verify no command sequence is in progress in SDHI, verify SD_INFO2.CBSY
   * is not set.  To verify the card has completed the requested operation,
   * verify SD_INFO2.SDD0MON is set. */
  FSP_ERROR_RETURN(
      SDHI_PRV_SD_INFO2_CBSY_SDD0MON_IDLE_VAL ==
          (p_ctrl->p_reg->SD_INFO2 & SDHI_PRV_SD_INFO2_CBSY_SDD0MON_IDLE_MASK),
      FSP_ERR_DEVICE_BUSY);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
static void r_sdhi_irq_enable(IRQn_Type irq, uint8_t priority,
                              void *p_context) {
  if (irq >= 0) {
...
static void r_sdhi_access_irq_process(sdhi_instance_ctrl_t *p_ctrl,
  ...
  if (flags.bit.response_end) {
    ...
    if (1U == p_ctrl->p_reg->SD_STOP_b.SEC) {
      /* Get the R1 response for multiple block read and write from SD_RSP54
       * since the response in SD_RSP10 may have been overwritten by the
       * response to CMD12. */
    ...
    if (SDHI_PRV_CMD_ERASE == p_ctrl->p_reg->SD_CMD) {
      /* Determine if erase is complete or not based on DAT0. Access interrupt
       * is not required for erase. */
      if (SDHI_PRV_SD_INFO2_CBSY_SDD0MON_IDLE_VAL ==
          (p_ctrl->p_reg->SD_INFO2 &
           SDHI_PRV_SD_INFO2_CBSY_SDD0MON_IDLE_MASK)) {
      ...
      /* Disable response end interrupt (set the bit) and enable access end
       * interrupt (clear the bit). */
  ...
  if (flags.word & SDHI_PRV_ACCESS_ERROR_MASK) {
    ...
    if (flags.bit.access_end) {
      /* All aligned transfers end here. Unaligned write transfers also end
       * here. Unaligned read transfers end in the transfer callback. */
      if (SDHI_TRANSFER_DIR_READ != p_ctrl->transfer_dir) {
        /* Disable the transfer and clear related variables since the transfer
         * is complete. */
        r_sdhi_transfer_end(p_ctrl);
  ...
  /* Combine all events for each command because this flag is polled in some
   * functions. */
...
static fsp_err_t r_sdhi_command_send(sdhi_instance_ctrl_t *p_ctrl,
  ...
  return r_sdhi_wait_for_event(p_ctrl, SDHI_PRV_RESPONSE_BIT,
                               SDHI_PRV_RESPONSE_TIMEOUT_US);
}
...
static fsp_err_t r_sdhi_max_clock_rate_set(sdhi_instance_ctrl_t *p_ctrl,
  ...
  /* Iterate over all possible divisors, starting with the smallest, until the
   * resulting clock rate is less than or equal to the requested maximum rate.
   */
  for (uint32_t divisor_shift = BSP_FEATURE_SDHI_MIN_CLOCK_DIVISION_SHIFT;
    ...
    if ((frequency >> divisor_shift) <= max_rate) {
      /* If the calculated frequency is less than or equal to the maximum
       * supported by the device, select this frequency. The register setting is
       * the divisor value divided by 4, or 0xFF for no divider. */
      setting = divisor_shift ? ((1U << divisor_shift) >> 2U) : UINT8_MAX;

      /* Set the clock setting. */

      /* The clock register is accessible 8 SD clock counts after the last
       * command completes.  Each register access requires at least one PCLK
       * count, so check the register up to 8 times the maximum PCLK divisor
       * value (512). */
      ...
      while (timeout > 0U) {
        ...
        if (p_ctrl->p_reg->SD_INFO2_b.SD_CLK_CTRLEN) {
          /* Set the calculated divider and enable clock output to start the 74
           * clocks required before initialization. Do not change the automatic
           * clock control setting. */
          uint32_t clkctrlen = p_ctrl->p_reg->SD_CLK_CTRL &
                               SDHI_PRV_SDHI_PRV_SD_CLK_CTRL_CLKCTRLEN_MASK;
          p_ctrl->p_reg->SD_CLK_CTRL =
              setting | clkctrlen | SDHI_PRV_SDHI_PRV_SD_CLK_CTRL_CLKEN_MASK;
...
static fsp_err_t r_sdhi_hw_cfg(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  /* Execute software reset or check SD_INFO2.SD_CLK_CTRLEN prior to calling
   * this function. */
  p_ctrl->p_reg->SD_CLK_CTRL =
      SDHI_PRV_SD_CLK_CTRL_DEFAULT; // Automatic clock control disabled.
  ...
  fsp_err_t err =
      r_sdhi_max_clock_rate_set(p_ctrl, SDHI_PRV_INIT_MAX_CLOCK_RATE_HZ);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

  /* Set initial bus width to one bit wide. */
  p_ctrl->p_reg->SD_OPTION =
      SDHI_PRV_SD_OPTION_DEFAULT |
      (SDHI_PRV_BUS_WIDTH_1_BIT << SDHI_PRV_SD_OPTION_WIDTH8_BIT);

  /* The host shall supply at least 74 SD clocks to the SD card while keeping
   * CMD line high.  Reference section 6.4.1.1 "Power Up Time of Card" in the SD
   * Physical Layer Specification Version 6.00. */
  R_BSP_SoftwareDelay(1U, BSP_DELAY_UNITS_MILLISECONDS);

  /* Automatic clock control can be enabled only after 74 SD/MMC clock cycles
   * are output.  Reference section 43.4.3 Automatic Control of SD/MMC Clock
   * Output (SD/MMC) of the RA6M3 manual R01UH0886EJ0100. */
...
static fsp_err_t r_sdhi_card_identify(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  /* For SDIO, follow the procedure in Figure 3-2 "Card Initialization Flow in
   * SD mode (SDIO Aware Host)" in SDIO Simplified Specification Version 3.00.
   */

  /* Reset I/O: In order to reset an I/O only card or the I/O portion of a combo
   * card, use CMD52 to set the RES bit in the CCCR (bit 3 of register 6).
   * Reference Table 6-2 "CCCR bit definitions" in SDIO Simplified Specification
   * Version 3.00. */
  uint8_t data = 1U << 3;
  r_sdhi_cmd52(p_ctrl, &data, 0U, 6U, SDMMC_IO_WRITE_MODE_NO_READ,
               SDHI_PRV_SDIO_CMD52_WRITE);
#endif

  /* For SD cards, follow the procedure in Figure 4-1 "SD Memory Card State
   * Diagram (card identification mode" in Physical Layer Simplified
   * Specification Version 6.00. */

  /* For eMMC devices, follow the procedure in A.6.1 "Bus Initialization" in
...
#endif
  {
    ...
    /* If the device is not SDIO, check to see if it is an SD memory card (CMD8
     * + ACMD41). NOTE: Not supporting memory on SDIO combo cards. */
...
#endif
    {
      ...
      /* If the device is not SDIO or SD memory, check to see if it is an eMMC
       * device (CMD1). */
      ...
      if ((sdmmc_card_type_t)UINT8_MAX == p_ctrl->device.card_type) {
        ...
        /* If the device is not identified as SDIO, SD memory card, or eMMC,
         * return an error. */
        return FSP_ERR_CARD_INIT_FAILED;
      }
    }
    ...
    err = r_sdhi_command_send(p_ctrl, SDHI_PRV_CMD_ALL_SEND_CID,
                              0); /* send SD CMD2 */
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
#endif
  }
...
static fsp_err_t r_sdhi_bus_cfg(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  /* Set clock rate to highest supported by both host and device.  Move card to
   * transfer state during this process. */
  if (p_ctrl->device.card_type == SDMMC_CARD_TYPE_SDIO) {
    ...
    /* Set bus width.
     * Note: Low speed SDIO not supported, so no need to check if 4-bit mode is
     * supported. */
    uint8_t bus_width_setting = (uint8_t)p_ctrl->p_cfg->bus_width;
    bus_width_setting = ((bus_width_setting >> 1) & 0x03U);
    err = r_sdhi_cmd52(p_ctrl, &bus_width_setting, 0U, 0x07U,
    ...
    /* Enable SDIO interrupts. Busy flag must be cleared before enabling
     * interrupts. */
...
#endif
  {
    ...
    err = r_sdhi_command_send(p_ctrl, SDHI_PRV_CMD_SET_BLOCKLEN,
...
static fsp_err_t r_sdhi_cmd52(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  sdmmc_priv_sdio_arg_t argument = {0U};
  argument.cmd_52_arg.function_number =
      (function & SDHI_PRV_SDIO_CMD52_CMD53_FUNCTION_MASK);
  argument.cmd_52_arg.rw_flag = (command & 1U);
  argument.cmd_52_arg.raw = read_after_write;
  argument.cmd_52_arg.register_address =
      (address & SDHI_PRV_SDIO_CMD52_CMD53_ADDRESS_MASK);
  argument.cmd_52_arg.data = *p_data;

  /* Send CMD52. */
  fsp_err_t err =
      r_sdhi_command_send(p_ctrl, SDHI_PRV_CMD_IO_RW_DIRECT, argument.arg);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
static fsp_err_t r_sdhi_sdio_check(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  if (response.r4.io_functions) {
    /* If the card supports SDIO, check for the card to be ready for at least
     * one second. */

    /* To ensure the 1 second timeout, consider that there are 48 bits in a
     * command, 48 bits in a response, and 8 clock cycles minimum between
     * commands, so there are 104 clocks minimum, and the maximum clock rate at
     * this point is 400 kHz, so issue the command 400000 / 104 times to ensure
     * a timeout of at least 1 second. */
    for (uint32_t i = 0U; i < SDHI_PRV_INIT_ONE_SECOND_TIMEOUT_ITERATIONS;
         i++) {
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
static fsp_err_t r_sdhi_sd_card_check(sdhi_instance_ctrl_t *const p_ctrl) {
  /* CMD8 must be sent before ACMD41. Reference Figure 4-1 "SD Memory Card State
   * Diagram (card identification mode)" in the SD Physical Layer Specification
   * Version 6.00. */
  uint32_t argument =
      ((SDHI_PRV_IF_COND_VOLTAGE << 8) | SDHI_PRV_IF_COND_CHECK_PATTERN);
  fsp_err_t err = r_sdhi_command_send(p_ctrl, SDHI_PRV_CMD_IF_COND, argument);
  if (FSP_ERR_TIMEOUT == err) {
    FSP_RETURN(err);
  }

  /* An SD card responds to CMD8 by echoing the argument in the R7 response. An
   * eMMC device responds to CMD8 with the extended CSD. If the response does
   * not match the argument, return to check if this is an eMMC device. */
  ...
  if ((FSP_ERR_RESPONSE == err) || (response.status == argument)) {
    /* Try to send ACMD41 for up to 1 second as long as the card is responding
     * and initialization is not complete. Returns immediately if the card fails
     * to respond to ACMD41. */
    ...
    for (uint32_t i = 0U; i < SDHI_PRV_INIT_ONE_SECOND_TIMEOUT_ITERATIONS;
      ...
      /* ACMD41 */
      err = r_sdhi_command_send(
          p_ctrl, SDHI_PRV_CMD_C_ACMD | SDHI_PRV_CMD_SD_SEND_OP_COND, ocr);
      FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
static fsp_err_t r_sdhi_emmc_check(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  /* Tries to send CMD1 for up to 1 second as long as the device is responding
   * and initialization is not complete. Returns immediately if the device fails
   * to respond to CMD1. */

  /* To ensure the 1 second timeout, consider that there are 48 bits in a
   * command, 48 bits in a response, and 8 clock cycles minimum between
   * commands, so there are 104 clocks minimum, and the maximum clock rate at
   * this point is 400 kHz, so issue the command 400000 / 104 times to ensure a
  ...
  for (uint32_t i = 0U; i < SDHI_PRV_INIT_ONE_SECOND_TIMEOUT_ITERATIONS; i++) {
    ...
    fsp_err_t err =
        r_sdhi_command_send(p_ctrl, SDHI_PRV_EMMC_SEND_OP_COND, ocr);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
static void r_sdhi_write_protect_get(sdhi_instance_ctrl_t *const p_ctrl) {
  /* Update write protection status in the control block if the device is a
   * card. */
...
static fsp_err_t r_sdhi_clock_optimize(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  if (SDMMC_CARD_TYPE_MMC == p_ctrl->device.card_type) {
    ...
    if ((SDHI_PRV_EMMC_HIGH_SPEED_52_MHZ_BIT & device_type) > 0U) {
      ...
      err = r_sdhi_command_send(p_ctrl, SDHI_PRV_EMMC_CMD_SWITCH_WBUSY,
                                SDHI_PRV_EMMC_HIGH_SPEED_MODE);
      FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

      max_clock_rate = SDHI_PRV_EMMC_HIGH_SPEED_CLOCK_RATE; // Set clock rate to
                                                            // 52 MHz maximum
...
#endif
  {
    ...
    /* Ask SD card to switch to high speed if CMD6 is supported.  CMD6 is
     * supported if bit 10 of the CCC field in the CSD is set.*/
    if (SDHI_PRV_CSD_REG_CCC_CLASS_10_BIT ==
        (SDHI_PRV_CSD_REG_CCC_CLASS_10_BIT & p_csd_reg->csd_v1_b.ccc)) {
      if (FSP_SUCCESS == r_sdhi_sd_high_speed(p_ctrl)) {
        max_clock_rate = SDHI_PRV_SD_HIGH_SPEED_CLOCK_RATE; // Set clock rate to
                                                            // 50 MHz maximum
      }
    }
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
static fsp_err_t
r_sdhi_sdio_clock_optimize(sdhi_instance_ctrl_t *const p_ctrl) {
  fsp_err_t err;
  uint8_t data = 0U;
  ...
  err =
      r_sdhi_cmd52(p_ctrl, &data, 0U, SDHI_PRV_SDIO_REG_HIGH_SPEED,
                   SDMMC_IO_WRITE_READ_AFTER_WRITE, SDHI_PRV_SDIO_CMD52_WRITE);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

  /* Check to see if high speed mode was successfully enabled.  Both EHS and SHS
   * bits must be set. */
  uint8_t high_speed_enabled_mask = SDHI_PRV_SDIO_REG_HIGH_SPEED_BIT_EHS |
                                    SDHI_PRV_SDIO_REG_HIGH_SPEED_BIT_SHS;
  uint32_t max_clock_rate = SDHI_PRV_SD_DEFAULT_CLOCK_RATE;
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
static fsp_err_t r_sdhi_wait_for_event(sdhi_instance_ctrl_t *const p_ctrl,
                                       uint32_t bit, uint32_t timeout_us) {
  /* The event status is updated in the access interrupt.  Use a local copy of
   * the event status to make sure it isn't updated during the loop. */
...
static fsp_err_t r_sdhi_sd_high_speed(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  fsp_err_t err = r_sdhi_read_and_block(p_ctrl, SDHI_PRV_CMD_SWITCH,
                                        SDHI_PRV_SD_HIGH_SPEED_MODE_SWITCH,
                                        SDHI_PRV_SD_SWITCH_STATUS_SIZE);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

  /* Read the switch response to see if the high speed switch is supported and
  ...
  if (SDHI_PRV_SD_SWITCH_HIGH_SPEED_OK ==
      (p_read_data_8[SDHI_PRV_SD_SWITCH_HIGH_SPEED_RESPONSE] &
       SDHI_PRV_SD_SWITCH_HIGH_SPEED_OK)) {
    err = FSP_SUCCESS;
  }
...
static fsp_err_t r_sdhi_csd_save(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  /* Save sector count (total number of sectors on device) and erase sector
   * count (minimum erasable unit in sectors). */
  uint32_t mult;
  if ((SDHI_PRV_CSD_VERSION_1_0 == csd_version) ||
      (SDMMC_CARD_TYPE_MMC == p_ctrl->device.card_type)) {
    ...
    p_ctrl->device.sector_count =
        p_ctrl->device.sector_count * (read_sector_size / SDHI_MAX_BLOCK_SIZE);

    if (SDMMC_CARD_TYPE_MMC == p_ctrl->device.card_type) {
      /* If c_size is 0xFFF, then sector_count should be obtained from the
       * extended CSD. Set it to 0 to indicate it should come from the extended
       * CSD later. */
...
static fsp_err_t r_sdhi_csd_extended_get(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  fsp_err_t err = r_sdhi_read_and_block(p_ctrl, SDHI_PRV_EMMC_CMD_SEND_EXT_CSD,
                                        rca << 16, SDHI_PRV_EMMC_EXT_CSD_SIZE);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

  /* Store device type.  Also store sector count if it is not already
   * determined. */
  uint8_t *p_read_data_8 = (uint8_t *)&p_ctrl->aligned_buff[0];
  *p_device_type = p_read_data_8[SDHI_PRV_EMMC_EXT_CSD_DEVICE_TYPE_OFFSET];
  ...
  if (0U == p_ctrl->device.sector_count) {
    p_ctrl->device.sector_count =
        p_ctrl->aligned_buff[SDHI_PRV_EMMC_EXT_CSD_SEC_COUNT_OFFSET /
                             sizeof(uint32_t)];
  }
...
static fsp_err_t r_sdhi_read_and_block(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  fsp_err_t err =
      r_sdhi_transfer_read(p_ctrl, 1U, byte_count, &p_ctrl->aligned_buff[0]);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
  ...
  return r_sdhi_wait_for_event(p_ctrl, SDHI_PRV_ACCESS_BIT,
                               SDHI_PRV_ACCESS_TIMEOUT_US);
}
...
static fsp_err_t r_sdhi_rca_get(sdhi_instance_ctrl_t *const p_ctrl,
                                uint32_t *p_rca) {
  /* Send CMD3.  For eMMC, assign an RCA of SDHI channel number + 2. These bits
   * of the argument are ignored for SD cards. */
  uint32_t rca = (p_ctrl->p_cfg->channel + 2U);
  fsp_err_t err =
      r_sdhi_command_send(p_ctrl, SDHI_PRV_CMD_SEND_RELATIVE_ADDR, rca << 16);
  FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
static fsp_err_t r_sdhi_bus_width_set(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  if (SDMMC_CARD_TYPE_MMC == p_ctrl->device.card_type) {
    ...
    fsp_err_t err = r_sdhi_command_send(
        p_ctrl, SDHI_PRV_EMMC_CMD_SWITCH_WBUSY,
        ((0x1U << SDHI_PRV_SWITCH_ACCESS_SHIFT) |
         (SDHI_PRV_EMMC_BUS_WIDTH_INDEX << SDHI_PRV_SWITCH_INDEX_SHIFT) |
...
#endif
  {
    ...
    fsp_err_t err =
        r_sdhi_command_send(p_ctrl, SDHI_PRV_CMD_APP_CMD, rca << 16);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

    /* For SD cards, set bus width using ACMD6. */
    err = r_sdhi_command_send(p_ctrl,
                              SDHI_PRV_CMD_C_ACMD | SDHI_PRV_CMD_SET_BUS_WIDTH,
                              bus_width_setting);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
#else
    FSP_PARAMETER_NOT_USED(rca);
...
void r_sdhi_transfer_callback(sdhi_instance_ctrl_t *p_ctrl) {
  ...
  if (p_ctrl->transfer_blocks_total != p_ctrl->transfer_block_current) {
    if (SDHI_TRANSFER_DIR_READ == p_ctrl->transfer_dir) {
      /* If the transfer is a read operation into an unaligned buffer, copy the
       * block read from the aligned buffer in the control block to the
       * application data buffer. */
      memcpy(p_ctrl->p_transfer_data, (void *)&p_ctrl->aligned_buff[0],
             p_ctrl->transfer_block_size);
    }

    if (SDHI_TRANSFER_DIR_WRITE == p_ctrl->transfer_dir) {
      /* If the transfer is a write operation from an unaligned buffer, copy the
       * next block to write from the application data buffer to the aligned
       * buffer in the control block. */
      memcpy((void *)&p_ctrl->aligned_buff[0], p_ctrl->p_transfer_data,
             p_ctrl->transfer_block_size);
    }
  ...
  if (p_ctrl->transfer_blocks_total == p_ctrl->transfer_block_current) {
    if (SDHI_TRANSFER_DIR_READ == p_ctrl->transfer_dir) {
      /* After the entire read transfer to an unaligned buffer is complete, read
       * transfer end and callback are performed in transfer interrupt to ensure
       * the last block is in the application buffer before the callback is
       * called. */
      r_sdhi_transfer_end(p_ctrl);
...
static fsp_err_t r_sdhi_transfer_read(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  /* When the SD_DMAEN.DMAEN bit is 1, set the SD_INFO2_MASK.BWEM bit to 1 and
   * the SD_INFO2_MASK.BREM bit to 1. */
  p_ctrl->p_reg->SD_INFO2_MASK |= SDHI_PRV_SD_INFO2_MASK_BREM_BWEM_MASK;
  p_ctrl->p_reg->SD_DMAEN = SDHI_PRV_SD_DMAEN_DMAEN_SET;

  uint32_t transfer_settings = (uint32_t)TRANSFER_MODE_BLOCK
                               << TRANSFER_SETTINGS_MODE_BITS;
  transfer_settings |= TRANSFER_ADDR_MODE_INCREMENTED
                       << TRANSFER_SETTINGS_DEST_ADDR_BITS;
  transfer_settings |= TRANSFER_SIZE_4_BYTE << TRANSFER_SETTINGS_SIZE_BITS;

#if SDMMC_CFG_UNALIGNED_ACCESS_ENABLE

  /* If the pointer is not 4-byte aligned or the number of bytes is not a
   * multiple of 4, use a temporary buffer. Data will be transferred from the
   * temporary buffer into the user buffer in an interrupt after each block
   * transfer. */
  ...
  fsp_err_t err = p_ctrl->p_cfg->p_lower_lvl_transfer->p_api->reconfigure(
      p_ctrl->p_cfg->p_lower_lvl_transfer->p_ctrl,
...
static fsp_err_t r_sdhi_transfer_write(sdhi_instance_ctrl_t *const p_ctrl,
  ...
  if ((0U != ((uint32_t)p_data & 0x3U)) || (0U != (bytes & 3U))) {
    ...
    /* If the pointer is not 4-byte aligned or the number of bytes is not a
     * multiple of 4, use a temporary buffer. Transfer the first block to the
     * temporary buffer before enabling the transfer.  Subsequent blocks will be
     * transferred from the user buffer to the temporary buffer in an interrupt
...
static void r_sdhi_transfer_end(sdhi_instance_ctrl_t *const p_ctrl) {
  ...
  p_ctrl->p_cfg->p_lower_lvl_transfer->p_api->disable(
      p_ctrl->p_cfg->p_lower_lvl_transfer->p_ctrl);
...
static void r_sdhi_call_callback(sdhi_instance_ctrl_t *p_ctrl,
                                 sdmmc_callback_args_t *p_args) {
  /* Call user callback if provided, if an event was determined, and if the
   * driver is initialized. */
  if (NULL != p_ctrl->p_callback) {
    sdmmc_callback_args_t args;

    /* Store callback arguments in memory provided by user if available.  This
     * allows callback arguments to be stored in non-secure memory so they can
     * be accessed by a non-secure callback function. */
    ...
    if (!cmse_is_nsfptr(p_ctrl->p_callback)) {
      ...
      sdhi_prv_ns_callback p_callback =
          (sdhi_prv_ns_callback)(p_ctrl->p_callback);
...
void sdhimmc_accs_isr(void) {
  ...
  sdhi_instance_ctrl_t *p_ctrl =
      (sdhi_instance_ctrl_t *)R_FSP_IsrContextGet(irq);
  ...
  /* Clearing the IR bit must be done after clearing the interrupt source in the
   * the peripheral */
...
void sdhimmc_dma_req_isr(void) {
  ...
  /* Clear the IR flag in the ICU.
   * This must be after the callback because the next DTC transfer will begin
